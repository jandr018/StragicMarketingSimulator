...
class Stub_AdminController 
{
	...
	{	
		...
		$test1 = $adminController->CreatePage("");
		$test2 = $adminController->CreatePage(null);
		$test3 = $adminController->CreatePage(-5);
		$test4 = $adminController->CreatePage("text");
		$ans[0] = $this->CreatePage("");
		$ans[1] = $this->CreatePage(null);
		$ans[2] = $this->CreatePage(-5);
		$ans[3] = $this->CreatePage("text");
		
		
		$result = "Test 1: AdminController.CreatePage_Test("")".
				"\nDescription: Calling CreatePage with no args".
				"\nResult: ".((strcasecmp($test1,$ans[0]) == 0) ? 'True'|'False');
		$result .= "\n\nTest 2: AdminController.CreatePage_Test(null)".
				"\nDescription: Calling CreatePage with a null arg".
				"\nResult: ".((strcasecmp($test2,$ans[1]) == 0) ? 'True'|'False');
		$result .= "\n\nTest 3: AdminController.CreatePage_Test(-5)".
				"\nDescription: Calling CreatePage with an int arg".
				"\nResult: ".((strcasecmp($test3,$ans[2]) == 0) ? 'True'|'False');
		$result .= "\n\nTest 4: AdminController.CreatePage_Test('test')".
				"\nDescription: Calling CreatePage with a normal arg".
				"\nResult: ".((strcasecmp($test4,$ans[3]) == 0) ? 'True'|'False');
	...
	function addAdminPage_Test()
	{
		...
		$test1 = $adminController->addAdminPage();
		$ans = $this->addAdminPage("");

		$result = "\n\nTest 5: AdminController.addAdminPage_Test()".
				"\nDescription: Calling addAdminPage with no args".
				"\nResult: ".((strcasecmp($test1,$ans[0]) == 0) ? 'True'|'False');
	...
	function addBotHotelPage()
	{
		...
		$result = 	"<table class='table'>".
					...
					"<input type = 'Text' name = 'textbox_Hotel_game_id' require pattern='[0-9]'>".
	...
	{		
		...
		print_r($mydatabase->addBotStudent(strval($int), $fname, $lname, $email, 1   , intval($hotelID),"B0T20", "what are you trying to do?", "MARKETSIM", 1));
